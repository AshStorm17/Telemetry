...
class CustomTopology(Topo):
    ...
    def __init__(self):
        ...
        h32 = self.addHost(f'h32', ip=f'10.0.4.13/24')
        for i in range(1, 9):
            if i==2:
                continue
        ...
        self.addLink(s1_cc3,fw1,intfName2='fw1-eth0')
        self.addLink(fw1,h32,intfName1='fw1-eth1')
...
def simpleTest():
    ...
    fw1.cmd('ip link set fw1-eth0 up')
    fw1.cmd('ip link set fw1-eth1 up')
    ...
    fw1.cmd('ifconfig fw1-eth0 10.0.3.50/24')
    fw1.cmd('ifconfig fw1-eth1 10.0.4.2/24')
    ...
    for i in range(1, 9):
        ...
        if i <= 4:
            if i==2:
                h.cmd('ip route add default via 10.0.4.2')
            h.cmd('ip route add default via 10.0.3.200')
        else:
            h.cmd('ip route add default via 10.0.3.201')
    
    r1.cmd('ip route add 10.0.3.0/24 via 10.0.2.202') 
    r1.cmd('ip route add 10.0.4.0/24 via 10.0.2.202')
    r2.cmd('ip route add 10.0.1.0/24 via 10.0.2.200') 
    r2.cmd('ip route add 10.0.4.13 via 10.0.3.50 dev r2-eth2')

    fw1.cmd('ip route add 10.0.4.0/24 via 10.0.3.50 dev fw1-eth1')
    fw1.cmd('ip route add 10.0.1.0/24 via 10.0.3.200 dev fw1-eth0')
    fw1.cmd('ip route add 10.0.2.0/24 via 10.0.3.200 dev fw1-eth0')
    fw1.cmd('ip route add 10.0.3.0/24 via 10.0.3.200 dev fw1-eth0')
    fw1.cmd('ip route add default via 10.0.3.200')

    fw1.cmd('iptables -A FORWARD -s 10.0.1.18 -d 10.0.4.13 -m state --state NEW -j DROP') #h17
    fw1.cmd('iptables -A FORWARD -s 10.0.2.19 -d 10.0.4.13 -m state --state NEW -j DROP') #h28
    ...
    while True:
        ...
        try:
            ...
            print(f"Firewall 1 sent firewall rule details to cc3")
            time.sleep(0.1)
